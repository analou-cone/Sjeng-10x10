...
void setup_epd_line(char* inbuff) {
    ...
    static int rankoffsets[] = {A10, A9, A8, A7, A6, A5, A4, A3, A2, A1}; //x
    ...
    while((inbuff[i] != '\n') && (inbuff[i] != '\0')) {
        if(stage == 0 && isdigit(inbuff[i])) {
            ...
            if (stage == 1) {
                ...
                moved[B1] = moved[I1] = moved[F1] = 1; //x
                moved[B10] = moved[I10] = moved[F10] = 1; //x
                ...
                if (inbuff[i] == '-') {
                    ...
                    ep_square = ((norm_rank * 14) + A1) + (norm_file);		  
                }
            } else if (stage == 4) {
                /* ignore this for now */
            } else if (stage == 5) {
