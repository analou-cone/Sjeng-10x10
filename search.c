...
long int qsearch (int alpha, int beta, int depth) {
    /* perform a quiescense search on the current node using alpha-beta with
       negamax search */
...
move_s search_root (int originalalpha, int originalbeta, int depth) {
    ...
    if (is_draw ()) {
        printf ("Found draw by 3-fold repetition\n");
        result = draw_by_rep;
        cur_score = 0;
        pv_length[ply] = 0;
        return (dummy);
    ...
    while (remove_one (&i, &move_ordering[0], num_moves)) {
        if (!alllosers && rootlosers[i] && ((Variant == Losers) || (Variant == Suicide)))
            continue;
    ...
    if (no_moves && !is_pondering) {
        if (Variant != Suicide && Variant != Losers) {
            if (in_check ()) {
                ...
                printf ("no_moves, not pondering, not in check, stalemate!\n");
                result = stalemate;
            }
        } else {
            if (white_to_move == 1) {
